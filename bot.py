...
# Load environment variables from .env file (harmless no-op if none exists,
# e.g. on Railway where variables are injected directly into the environment)
load_dotenv()

def _clean_env_value(value):
    """Strip whitespace/newlines and matching surrounding quotes.
    Guards against the #1 cause of 'invalid token' on platforms like Railway:
    pasting a value with stray quotes/spaces/newlines into the dashboard."""
    if value is None:
        return value
    v = value.strip()
...
def env(name, default=None):
    """os.getenv() plus automatic stripping of whitespace/newlines and
    surrounding quotes — safe whether or not you quote your .env values."""
...
BOT_NAME = env('BOT_NAME', 'Kaidrixvpz VMS')
PREFIX = env('PREFIX', '!')
YOUR_SERVER_IP = env('YOUR_SERVER_IP')
MAIN_ADMIN_ID = env_int('MAIN_ADMIN_ID', '1210291131301101618')
...
PTERODACTYL_APP_API_KEY = env('PTERODACTYL_APP_API_KEY', '')
VPS_USER_ROLE_ID = env_int('VPS_USER_ROLE_ID', '1210291131301101618')
DEFAULT_STORAGE_POOL = env('DEFAULT_STORAGE_POOL', 'default')
HOST_MOTD = env('HOST_MOTD', '')
BOT_VERSION = env('BOT_VERSION', '8.0-PRO')
BOT_DEVELOPER = env('BOT_DEVELOPER', 'Kaidrixvpz')
BOT_THUMBNAIL_URL = env('BOT_THUMBNAIL_URL', 'https://i.imgur.com/Tv3clt0.jpeg')
BOT_ICON_URL = env('BOT_ICON_URL', 'https://i.imgur.com/Tv3clt0.jpeg')

# VPS Expiration Settings
DEFAULT_VPS_EXPIRATION_DAYS = env_int('DEFAULT_VPS_EXPIRATION_DAYS', '30')
EXPIRATION_WARNING_DAYS = env_int('EXPIRATION_WARNING_DAYS', '1')
...
DEPLOY_SLOT    = env_int('DEPLOY_SLOT', '0')
...
def create_embed(title, description="", color=COLOR_PRIMARY):
    ...
    embed.set_footer(
        text=f"Made by Kaidrix • v{BOT_VERSION} • {datetime.now().strftime('%H:%M:%S')}",
...
DEFAULT_STORAGE_POOL = env('DEFAULT_STORAGE_POOL', get_default_storage_pool())
...
async def my_vps(ctx):
    ...
    embed.set_footer(text=f"Made by Kaidrix • VPS Control Panel")
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        # Example: kaidrix-vps-1, alexuser-vps-2, btw-infinite-vps-3
...
async def vps_list(ctx, node_id: int = 1):
    ...
    if vps_info:
        ...
        for idx, chunk in enumerate(chunks[1:], 2):
            ...
            page_embed.set_footer(text=f"Made by Kaidrix • {len(vps_info)} VPS shown")
            await ctx.send(embed=page_embed)
    else:
        add_field(embed, "📋 **VPS List**", "No deployments yet. Launch one! 🚀", False)

    embed.set_footer(text=f"Made by Kaidrix • Total: {len(vps_info)} VPS")
    await ctx.send(embed=embed)
...
async def system_status(ctx):
    ...
    embed.set_footer(text=f"Made by Kaidrix • System Status • Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                    icon_url=BOT_ICON_URL)
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.set_footer(text="Made by Kaidrix • User Resource Dashboard")
    embed.timestamp = ctx.message.created_at
...
async def server_stats(ctx):
    ...
    embed.set_footer(text="Made by Kaidrix • Real-Time Monitoring")
    embed.timestamp = ctx.message.created_at
...
async def vps_info(ctx, container_name: str = None):
    if not container_name:
        ...
        for idx, chunk in enumerate(chunks, 1):
            ...
            embed.set_footer(text=f"Made by Kaidrix • VPS Information System")
            await ctx.send(embed=embed)
    else:
        found_vps = None
        found_user = None
        ...
        embed.set_footer(text=f"Made by Kaidrix • VPS Information System • Container: {container_name}")
        await ctx.send(embed=embed)
...
async def vps_password(ctx, container_name: str = None):
    ...
    if not container_name:
        ...
        for idx, chunk in enumerate(chunks, 1):
            ...
            embed.set_footer(text=f"Made by Kaidrix • Password Management")
            await ctx.send(embed=embed)
    else:
        ...
        embed.set_footer(text=f"Made by Kaidrix • Password Information")
        await ctx.send(embed=embed)
...
async def quick_help(ctx):
    ...
    embed.set_footer(text=f"Made by Kaidrix • Use {PREFIX}help for complete command list")
    await ctx.send(embed=embed)
...
async def help_search(ctx, *, search_term: str = None):
    ...
    embed.set_footer(text=f"Made by Kaidrix • Use {PREFIX}help for complete list")
    await ctx.send(embed=embed)    
...
if __name__ == "__main__":
    if not DISCORD_TOKEN or DISCORD_TOKEN == 'your_discord_bot_token_here':
        ...
        logger.error("Set DISCORD_TOKEN in your .env file, or in Railway's Variables tab if deploying there.")
        exit(1)

    # Sanity-check the token shape without ever logging the token itself.
    token_len = len(DISCORD_TOKEN)
    has_whitespace = any(c.isspace() for c in DISCORD_TOKEN)
    has_quotes = ('"' in DISCORD_TOKEN) or ("'" in DISCORD_TOKEN)
    starts_with_bot_prefix = DISCORD_TOKEN.lower().startswith("bot ")
    logger.info(f"Discord token loaded (length={token_len}). Real bot tokens are ~59-72 chars with no spaces/quotes.")
    if has_whitespace or has_quotes or starts_with_bot_prefix or token_len < 50:
        logger.warning(
            "⚠️ This token looks off (contains whitespace/quotes, has a 'Bot ' prefix, or is unusually short). "
            "This is the most common cause of Discord rejecting a token that 'looks right' when you view it — "
            "invisible characters or an extra 'Bot ' prefix copied along with it. "
            "On Railway: open your service -> Variables -> DISCORD_TOKEN, delete it, and re-add it by pasting "
            "ONLY the token from the Discord Developer Portal (no quotes, no 'Bot ' prefix, no trailing newline)."
        )

    try:
        bot.run(DISCORD_TOKEN)
    except discord.errors.LoginFailure as e:
        logger.error(f"❌ ERROR: Discord rejected this token: {e}")
        logger.error("This means Discord itself considers the token invalid/revoked — it is NOT related to "
                     "LXC/VPS hosting or which platform (Railway, a VPS, etc.) is running this process.")
        logger.error("Most common causes:")
        logger.error("  1. The token was regenerated in the Discord Developer Portal after you copied the old one "
                     "(regenerating invalidates the previous token immediately).")
        logger.error("  2. The DISCORD_TOKEN variable name is slightly different in your platform's dashboard "
                     "(these are case-sensitive) — it must be exactly DISCORD_TOKEN.")
        logger.error("  3. A leftover .env file in your deployment still has a placeholder/old value AND your "
                     "platform isn't actually injecting its own DISCORD_TOKEN variable at all — check that the "
                     "variable is set at the platform level, not only in a local .env you forgot to update.")
        logger.error("  4. The 'Client Secret' or 'Application ID' was pasted instead of the actual Bot Token "
